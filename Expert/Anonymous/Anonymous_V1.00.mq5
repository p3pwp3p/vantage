...
input double EURUSDTradeMarginPercent = 170;
input int EURUSDMartingalePointGap = 1000;
input double EURUSDLotsMultiple = 1.41;
...
input ENUM_ORDER_FILLING OrderFilling = 0;
...
input double GBPAUDLotsMultiple = 1.41;
...
EURUSDTradeValidator::EURUSDTradeValidator() {
    ...
    buyLotSize = Lots;
    sellLotSize = Lots;
    totalBars = iBars("EURUSD", PERIOD_CURRENT);
}
...
bool EURUSDTradeValidator::checkHistory(int minimumBars) {
    ...
    if (Bars(symbol, PERIOD_CURRENT) < MinimumBars) {
        logValidationInfo("WARNING: Not enough historical data. Required: " + IntegerToString(MinimumBars) +
                          ", Available: " + IntegerToString(Bars(symbol, PERIOD_CURRENT)));
        return false;
    }
...
void EURUSDTradeValidator::updateBuyLotSize(bool flag) {
    // trade ?
    if (flag) {
        buyLotSize = hasOpenBuyPositions() ? NormalizeDouble(buyLotSize * EURUSDLotsMultiple, 2) : marginPerLotSize;
    } else {
...
void EURUSDTradeValidator::updateSellLotSize(bool flag) {
    if (flag) {
        sellLotSize = hasOpenSellPositions() ? NormalizeDouble(sellLotSize * EURUSDLotsMultiple, 2) : marginPerLotSize;
    } else {
...
void EURUSDTradeValidator::updateTotalBars(int bars) {
    totalBars = bars;
}

bool EURUSDTradeValidator::validateLotSize() {
    double contractSize = SymbolInfoDouble(symbol, SYMBOL_TRADE_CONTRACT_SIZE);
    marginPerLotSize = MathCeil(balance / contractSize * (EURUSDTradeMarginPercent / 100) * 100) / 100 - 0.01;
    if (marginPerLotSize < minLotSize)
        return false;
    return true;
}

double EURUSDTradeValidator::validateStopLoss(ENUM_ORDER_TYPE type, double currentPrice) {
    if (currentPrice <= 0.0)
        return 0.0;
    if (SlPoints <= 0.0)
        return 0.0;

    bool isBuy = (type == ORDER_TYPE_BUY || type == ORDER_TYPE_BUY_LIMIT || type == ORDER_TYPE_BUY_STOP ||
                  type == ORDER_TYPE_BUY_STOP_LIMIT);
    bool isSell = (type == ORDER_TYPE_SELL || type == ORDER_TYPE_SELL_LIMIT || type == ORDER_TYPE_SELL_STOP ||
                   type == ORDER_TYPE_SELL_STOP_LIMIT);

    if (isBuy)
        return NormalizeDouble(currentPrice - (double)SlPoints * _Point, _Digits);
    if (isSell)
        return NormalizeDouble(currentPrice + (double)SlPoints * _Point, _Digits);
...
double EURUSDTradeValidator::validateTakeProfit(ENUM_ORDER_TYPE type, double currentPrice) {
    if (currentPrice <= 0.0)
        return 0.0;
    if (TpPoints <= 0.0)
        return 0.0;
                ...
                for (int i = 0; i < PositionsTotal(); i++) {
                    ...
                    if (PositionSelectByTicket(posTicket)) {
                        if (PositionGetString(POSITION_SYMBOL) == symbol && posType == POSITION_TYPE_BUY) {
                            ...
                            if (sl > posSl) {
                                if (trade.PositionModify(posTicket, sl, posTp)) {
                                    Print(__FUNCTION__ " > Position ticket # ", posTicket,
...
bool EURUSDTradeValidator::executeTrade(ENUM_ORDER_TYPE type, double currentPrice, double volume, ulong magic) {
...
void EURUSDTradeValidator::closePositions(ENUM_ORDER_TYPE type, ENUM_POSITION_TYPE positionType) {
    for (int i = 0; i < PositionsTotal(); i++) {
        ENUM_POSITION_TYPE ptype = positionInfo.PositionType();
...
GBPAUDTradeValidator::GBPAUDTradeValidator() {
    ...
    buyLotSize = Lots;
    sellLotSize = Lots;
    totalBars = iBars("GBPAUD", PERIOD_CURRENT);
}
...
void GBPAUDTradeValidator::updateBuyLotSize(bool flag) {
    // trade ?
    if (flag) {
        buyLotSize = hasOpenBuyPositions() ? NormalizeDouble(buyLotSize * GBPAUDLotsMultiple, 2) : marginPerLotSize;
    } else {
        buyLotSize = hasOpenBuyPositions() ? buyLotSize : marginPerLotSize;
    }
}
...
void GBPAUDTradeValidator::updateSellLotSize(bool flag) {
    if (flag) {
        sellLotSize = hasOpenSellPositions() ? NormalizeDouble(sellLotSize * GBPAUDLotsMultiple, 2) : marginPerLotSize;
    } else {
        sellLotSize = hasOpenSellPositions() ? sellLotSize : marginPerLotSize;
    }
}
...
void GBPAUDTradeValidator::updateTotalBars(int bars) {
    totalBars = bars;
}

bool GBPAUDTradeValidator::validateLotSize() {
    double contractSize = SymbolInfoDouble(symbol, SYMBOL_TRADE_CONTRACT_SIZE);
    marginPerLotSize = MathCeil(balance / contractSize * (GBPAUDTradeMarginPercent / 100) * 100) / 100 - 0.01;
    if (marginPerLotSize < minLotSize)
        return false;
    return true;
}

double GBPAUDTradeValidator::validateStopLoss(ENUM_ORDER_TYPE type, double currentPrice) {
    if (currentPrice <= 0.0)
        return 0.0;
    if (SlPoints <= 0.0)
        return 0.0;
...
double GBPAUDTradeValidator::validateTakeProfit(ENUM_ORDER_TYPE type, double currentPrice) {
    if (currentPrice <= 0.0)
        return 0.0;
    if (TpPoints <= 0.0)
        return 0.0;
...
bool GBPAUDTradeValidator::executeTrade(ENUM_ORDER_TYPE type, double currentPrice, double volume, ulong magic) {
    MqlTradeRequest request = {};
    MqlTradeResult result = {};
...
void GBPAUDTradeValidator::closePositions(ENUM_ORDER_TYPE type, ENUM_POSITION_TYPE positionType) {
    for (int i = 0; i < PositionsTotal(); i++) {
        ENUM_POSITION_TYPE ptype = positionInfo.PositionType();
        if (ptype == positionType && positionInfo.Symbol() == symbol && positionInfo.Magic() == MagicNumber) {
            trade.PositionClose(positionInfo.Ticket(), 0);
...
int OnInit() {
    eurusdHandleFastMA = iMA("EURUSD", PERIOD_M15, FastMAPeriod, 0, MAMode, AppliedPrice);
    eurusdHandleSlowMA = iMA("EURUSD", PERIOD_M15, SlowMAPeriod, 0, MAMode, AppliedPrice);

    gbpaudHandleFastMA = iMA("GBPAUD", PERIOD_M15, FastMAPeriod, 0, MAMode, AppliedPrice);
    gbpaudHandleSlowMA = iMA("GBPAUD", PERIOD_M15, SlowMAPeriod, 0, MAMode, AppliedPrice);

    if (eurusdHandleFastMA == INVALID_HANDLE || eurusdHandleSlowMA == INVALID_HANDLE ||
        gbpaudHandleFastMA == INVALID_HANDLE || gbpaudHandleSlowMA == INVALID_HANDLE) {
...
void OnTick() {
    ...
    if (eurusdCurrentBars != EURUSDvalidator.getTotalBars()) {
        ...
        if (CopyBuffer(eurusdHandleFastMA, 0, 1, IndicatorStopTime, eurusdFastMA) < IndicatorStopTime ||
            CopyBuffer(eurusdHandleSlowMA, 0, 1, IndicatorStopTime, eurusdSlowMA) < IndicatorStopTime) {
            Print("Failed copying indicator values. ", GetLastError());
            return;
        }

        bool buySignal =
            eurusdFastMA[PREVIOUS] < eurusdSlowMA[PREVIOUS] && eurusdFastMA[CURRENT] > eurusdSlowMA[CURRENT];
        bool sellSignal =
            eurusdFastMA[PREVIOUS] > eurusdSlowMA[PREVIOUS] && eurusdFastMA[CURRENT] < eurusdSlowMA[CURRENT];

        if (sellSignal) {
            // sell signal
            if (eurusdHasOpenSellPositions && EURUSDvalidator.getLastSellPrice() <
                                                  eurusdAsk - EURUSDMartingalePointGap * EURUSDvalidator.getPoint()) {
                double volume = EURUSDvalidator.getSellLotSize();
                EURUSDvalidator.executeTrade(ORDER_TYPE_SELL, eurusdAsk, volume, MagicNumber);
        ...
        if (buySignal) {
            ...
            if (eurusdHasOpenBuyPositions &&
                EURUSDvalidator.getLastBuyPrice() > eurusdBid + EURUSDMartingalePointGap * _Point) {
                double volume = EURUSDvalidator.getBuyLotSize();
                EURUSDvalidator.executeTrade(ORDER_TYPE_BUY, eurusdBid, volume, MagicNumber);
    ...
    if (gbpaudCurrentBars != GBPAUDvalidator.getTotalBars()) {
        ...
        if (CopyBuffer(gbpaudHandleFastMA, 0, 1, IndicatorStopTime, gbpaudFastMA) < IndicatorStopTime ||
            CopyBuffer(gbpaudHandleSlowMA, 0, 1, IndicatorStopTime, gbpaudSlowMA) < IndicatorStopTime) {
            Print("Failed copying indicator values. ", GetLastError());
            return;
        }

        bool buySignal =
            gbpaudFastMA[PREVIOUS] < gbpaudSlowMA[PREVIOUS] && gbpaudFastMA[CURRENT] > gbpaudSlowMA[CURRENT];
        bool sellSignal =
            gbpaudFastMA[PREVIOUS] > gbpaudSlowMA[PREVIOUS] && gbpaudFastMA[CURRENT] < gbpaudSlowMA[CURRENT];

        if (sellSignal) {
            // sell signal
            if (gbpaudHasOpenSellPositions && GBPAUDvalidator.getLastSellPrice() <
                                                  gbpaudAsk - GBPAUDMartingalePointGap * GBPAUDvalidator.getPoint()) {
                double volume = GBPAUDvalidator.getSellLotSize();
                GBPAUDvalidator.executeTrade(ORDER_TYPE_SELL, gbpaudAsk, volume, MagicNumber);
        ...
        if (buySignal) {
            ...
            if (gbpaudHasOpenBuyPositions &&
                GBPAUDvalidator.getLastBuyPrice() > gbpaudBid + GBPAUDMartingalePointGap * _Point) {
                double volume = GBPAUDvalidator.getBuyLotSize();
                GBPAUDvalidator.executeTrade(ORDER_TYPE_BUY, gbpaudBid, volume, MagicNumber);
