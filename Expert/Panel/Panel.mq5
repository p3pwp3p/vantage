...
input double EURUSDLotsMultiple = 2.33;
...
int OnInit() {
    long accountCustomer = 151524335;
    long accountNo = AccountInfoInteger(ACCOUNT_LOGIN);
    if (accountCustomer == accountNo) {
        Print(__FUNCTION__, "> verified.");
    }
    else {
        Print(__FUNCTION__, "Draw slope line cannot be used. Please use it after purchase.");
        ExpertRemove();
        return INIT_FAILED;
    }

    eurusdHandleFastMA =
        iMA("EURUSD", PERIOD_M15, FastMAPeriod, 0, MAMode, AppliedPrice);
    eurusdHandleSlowMA =
        iMA("EURUSD", PERIOD_M15, SlowMAPeriod, 0, MAMode, AppliedPrice);

    gbpaudHandleFastMA =
        iMA("GBPAUD", PERIOD_M15, FastMAPeriod, 0, MAMode, AppliedPrice);
    gbpaudHandleSlowMA =
        iMA("GBPAUD", PERIOD_M15, SlowMAPeriod, 0, MAMode, AppliedPrice);
