...
#property version "1.62"  // Performance Optimized
#property strict
...
input int EntryCooldownSeconds =
    3600;  // 진입 후 3600초(1시간) 동안 재진입 금지

input group "Box Breakout Settings";
input string Ex_Pend = "23:59";
input double EntryBufferPip = 30.0;
...
datetime lastEntryTime = 0;  // [신규] 마지막 진입 시간 기록용
...
void OnTick() {
    ...
    if (dt.hour >= TradeStartHour && dt.hour < TradeEndHour) {
        // [수정] 쿨타임 체크 추가 (마지막 진입 후 N초 지났는지 확인)
...
void checkBoxEntry() {
    ...
    // [중요] 주문이 하나라도 들어가면 쿨타임 시작
    if (orderPlaced) {
        lastEntryTime = currentTime;
        Print("✅ Orders Placed. Cooldown started for ", EntryCooldownSeconds,
              "s");
    }
}

// --- Helpers (나머지 동일) ---
// (기존 코드의 나머지 함수들: manageGridExit, checkOCO, manageGridOrders,
// manageRecoveryOrders, manageTrailingStop, placeRecoveryOrder,
// hasRecoveryOrder, getHighest, getLowest, closeAllPositions,
// deletePendingOrders, verifyVolume, hasGridOrder, hasEntryPendingOrders,
// performDailyReset, updateInfoPanel 등은 v1.61과 동일하게 유지)

// [복사 편의를 위해 생략된 부분은 v1.61 코드를 그대로 사용하세요. 변경점은
// OnTick과 checkBoxEntry, 그리고 전역변수 lastEntryTime 뿐입니다.]
...
void updateInfoPanel() {
    ...
    string info = StringFormat(
        "── [ QE Benchmark v1.62 (Opt) ] ──\nPositions: %d\nCooldown: %s",
        PositionsTotal(),
        (TimeCurrent() - lastEntryTime < EntryCooldownSeconds ? "Active"
                                                              : "Ready"));
    Comment(info);
}
void manageGridOrders() {
