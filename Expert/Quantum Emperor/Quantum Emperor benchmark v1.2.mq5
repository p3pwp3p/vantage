...
input string Box_Start = "00:00";
input int TradeStartHour = 10;
input bool UseDailyTrendFilter = false;

input group "Re-Entry Settings (New)";
input int MaxDailyCycles = 2;       // [신규] 하루 최대 진입 횟수 (기본 2회)
input int ReEntryCooldownMin = 30;  // [신규] 청산 후 재진입 대기 시간 (분)
...
datetime lastCloseTime = 0;  // 마지막 청산 시간 저장용
...
int getDailyDealsCount();  // [NEW] 오늘 거래 횟수 카운트
...
void OnTick() {
    ...
    // [필터 1] 시간 제한
    if (dt.hour < TradeStartHour) return;

    // [필터 2] 횟수 제한 (MaxDailyCycles)
    int dailyTrades = getDailyDealsCount();
    if (dailyTrades >= MaxDailyCycles) return;  // 횟수 다 썼으면 종료

    // [필터 3] 쿨타임 체크 (재진입 시)
    if (dailyTrades > 0) {
        // 마지막 청산 후 N분이 지났는지 확인
        if (TimeCurrent() < lastCloseTime + (ReEntryCooldownMin * 60)) return;
    }
    ...
    if (HistorySelect(startOfDay, now)) {
        for (int i = 0; i < HistoryDealsTotal(); i++) {
            ...
            if (HistoryDealGetInteger(ticket, DEAL_MAGIC) == InpMagicNumber &&
                ...
                // 같은 시간에 여러 개(3분할) 청산된 건 1회로 치기 위해 시간
                // 체크
                datetime dealTime =
                    (datetime)HistoryDealGetInteger(ticket, DEAL_TIME);
                if (dealTime >
                    lastTime + 60)  // 1분 이상 차이나면 별도 거래로 인정
                {
                    count++;
                    lastTime = dealTime;
                }
...
void checkBoxEntry() {
    ...
    datetime startTime = StringToTime(dateStr + " " + Box_Start);
    ...
    // 4. 주문 전송 (재진입 로그 출력)
    int currentCycle = getDailyDealsCount() + 1;
    Print("✅ Entry Attempt (Cycle ", currentCycle, ")! Box:", boxSizePip);
...
void performDailyReset() {
    if (lastAnalysisTime != iTime(Symbol(), PERIOD_D1, 0)) {
        ...
        deletePendingOrders();
        lastCloseTime = 0;  // 새 날 시작 시 쿨타임 리셋
    }
}
void updateInfoPanel() {
    if (!MQLInfoInteger(MQL_TESTER) && !MQLInfoInteger(MQL_VISUAL_MODE)) return;
    string info =
        StringFormat("── [ QE Benchmark v1.9 (Re-Entry) ] ──\nCycles: %d / %d",
