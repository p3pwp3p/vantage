...
void OnTick() {
    ...
    // 재진입 로직 (Dynamic Entry)
    MqlDateTime dt;
    TimeToStruct(TimeCurrent(), dt);

    // 대기 주문도 없고, 포지션도 없을 때 (완전 무포지션)
...
// ──────────────────────────────────────────────────────────────────
// [수정] Dynamic Box Entry (현재 시간까지의 고점/저점 갱신)
// ──────────────────────────────────────────────────────────────────
void checkBoxEntry() {
    datetime currentTime = TimeCurrent();
    string dateStr = TimeToString(currentTime, TIME_DATE);

    // [핵심 변경] 롤링 방식(N시간 전) -> 고정 시간 방식(03:00 ~ 11:00)
    // 영상 피셜: "Between 3am and 11am determines highs and lows"
    datetime startTime = StringToTime(dateStr + " " + Box_Start_Time);  // 03:00

    // 11시가 되면 박스 계산 끝. 진입 시작.
    // TradeStartHour(11)를 이용하여 종료 시간 설정
    datetime endTime =
        StringToTime(StringFormat("%s %02d:00", dateStr, TradeStartHour));

    // 아직 11시가 안 됐으면 계산하지 말고 대기
    ...
    // TP/SL (영상 피셜: 고정 600포인트)
    // ATR을 쓸 수도 있지만, 원본 재현을 위해 Fixed 사용 권장
    ...
    double tp1_real =
        tp1_pts - spread;  // TP는 스프레드만큼 줄여서 체결 확률 높임
    double tp2_real = tp2_pts - spread;

    // 주문 전송
    Print("✅ Static Box Entry (03-11)! High:", high, " Low:", low,
          " Box:", boxSizePip);
