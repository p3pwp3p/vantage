//+------------------------------------------------------------------+
//|                               Quantum Emperor benchmark v4.3     |
//|                               (Pullback & Re-Breakout)           |
//+------------------------------------------------------------------+
...
void checkStealthEntry() {
    ...
    // ──────────────────────────────────────────────────────────
    // [핵심 로직] 재진입 시 눌림목(Pullback) 체크
    // ──────────────────────────────────────────────────────────
    if (lockedDir != 0)  // 이미 오늘 거래를 한 번 했다면 (재진입 상황)
    {
        // 아직 눌림목 확인이 안 된 상태라면?
        if (!isPullbackDetected) {
            // 매수 방향: 가격이 박스 고점 아래로 내려와야 함
            if (lockedDir == 1 && bid < dailyHigh) {
                isPullbackDetected = true;
                Print(
                    "📉 Pullback Detected (Buy Side). Ready for Re-Breakout.");
            }
            // 매도 방향: 가격이 박스 저점 위로 올라와야 함
            else if (lockedDir == -1 && ask > dailyLow) {
                isPullbackDetected = true;
                Print(
                    "📈 Pullback Detected (Sell Side). Ready for Re-Breakout.");
            }
...
void executeEntry(ENUM_ORDER_TYPE type, double price) {
    ...
    isPullbackDetected =
        false;  // 진입했으니 플래그 초기화 (다음 재진입을 위해)
    Print("🚀 Stealth Entry Executed!");
}
...
// (나머지 Grid, Recovery, Helper 함수들은 v1.62와 100% 동일하므로 생략하지 않고
// 아래에 붙여넣습니다)
void updateInfoPanel() {
...
int getDailyDealsCount() {
    datetime s = iTime(Symbol(), PERIOD_D1, 0);
    int c = 0;
