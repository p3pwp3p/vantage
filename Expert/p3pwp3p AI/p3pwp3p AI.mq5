...
int OnInit() {
    ...
    // [중요] 입력 셰이프 변경: [1, 6] (RSI, ATR, ADX, Vol, Hour, Minute)
    long inputShape[] = {1, 6};
    ...
    Print("AI Trader V1.12 (Time Aware) Start");
...
void OnTick() {
    ...
    // [시간 정보 추출]
    MqlDateTime dt;
    TimeToStruct(iTime(_Symbol, _Period, 1), dt);  // 전 봉 기준 시간
    ...
    inputData[0][4] = (float)dt.hour;  // 시간
    inputData[0][5] = (float)dt.min;   // 분
