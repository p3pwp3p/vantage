...
static input int    Magic_Number = 1515361653; // Magic number
static input double Entry_Amount =     0.01; // Entry lots
       input int    Stop_Loss    =      779; // Stop Loss   (pips)
       input int    Take_Profit  =      919; // Take Profit (pips)
...
static input string ___2______   = "------"; // --- Stochastic ---
       input int    Ind2Param0   =        4; // %K Period
       input int    Ind2Param1   =        1; // %D Period
       input int    Ind2Param2   =        2; // Slowing
       input double Ind2Param3   =     50.0; // Level

static input string ___3______   = "------"; // --- Awesome Oscillator ---
       input double Ind3Param0   =   0.0000; // Level

static input string ___4______   = "------"; // --- Candle Color ---
       input int    Ind4Param0   =       49; // Min body height [pips]
       input int    Ind4Param1   =        5; // Consecutive candles

static input string ___5______   = "------"; // --- Moving Averages Crossover ---
       input int    Ind5Param0   =       15; // Fast MA period
       input int    Ind5Param1   =       18; // Slow MA period
...
void InitIndicators(void)
  {
   ...
   indHandlers[0][3][0] = iAO(NULL, 0);
   if(Show_inds) ChartIndicatorAdd(0, 4, indHandlers[0][3][0]);
   // Candle Color (49, 5)
   indHandlers[0][4][0] = -1;
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][4][0]);
   // Moving Averages Crossover (Simple, Simple, 15, 18)
   indHandlers[0][5][0] = iMA(NULL, 0, Ind5Param0, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][5][0]);
   // Moving Averages Crossover (Simple, Simple, 15, 18)
   indHandlers[0][5][1] = iMA(NULL, 0, Ind5Param1, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][5][1]);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
void ManageClose(void)
  {
   ...
   bool ind4short = false;
   {
      ...
      double pipVal = pip * Ind4Param0;

      for (int b = Ind4Param1 + 2; b > 0; b--)
        {
      ...
      ind4long  = consecutiveBearish >= Ind4Param1;
      ind4short = consecutiveBullish >= Ind4Param1;
   }

   // Moving Averages Crossover (Simple, Simple, 15, 18)
   double ind5buffer0[]; CopyBuffer(indHandlers[0][5][0], 0, 1, 2, ind5buffer0);
   double ind5buffer1[]; CopyBuffer(indHandlers[0][5][1], 0, 1, 2, ind5buffer1);
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET BlackBull; XAUUSD; H1 */
/*STRATEGY CODE {"properties":{"entryLots":0.01,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":779,"takeProfit":919,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Accelerator Oscillator","listIndexes":[7,0,0,0,0],"numValues":[0,0,0,0,0,0]},{"name":"Bears Power","listIndexes":[3,0,0,0,0],"numValues":[8,0,0,0,0,0]},{"name":"Stochastic","listIndexes":[0,0,0,0,0],"numValues":[4,1,2,50,0,0]},{"name":"Awesome Oscillator","listIndexes":[2,0,0,0,0],"numValues":[0,0,0,0,0,0]}],"closeFilters":[{"name":"Candle Color","listIndexes":[1,0,0,0,0],"numValues":[49,5,0,0,0,0]},{"name":"Moving Averages Crossover","listIndexes":[0,0,0,0,0],"numValues":[15,18,0,0,0,0]}]} */
