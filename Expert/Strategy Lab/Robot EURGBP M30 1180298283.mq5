/**
 * Top 10 Robots App Expert Advisor
 *
 * Exported from Top 10 Robots App
 * MQL base code updated on 2025-06-09
 * Website https://eatradingacademy.com/premium-robots/top-10-robots-app/
 *
 * Copyright 2025, EA Academy
 *
 * Risk Disclosure
 *
 * Futures and forex trading contains substantial risk and is not for every
 * investor. An investor could potentially lose all or more than the initial
 * investment. Risk capital is money that can be lost without jeopardizing ones’
 * financial security or life style. Only risk capital should be used for
 * trading and only those with sufficient risk capital should consider trading.
 */

#property copyright "EA Academy"
#property version "6.3"
#property strict

static input string _Properties_ = "------";  // --- Expert Properties ---
static input int Magic_Number = 1180298283;   // Magic number
static input double Entry_Amount = 0.01;      // Entry lots
input int Stop_Loss = 59;                     // Stop Loss   (pips)
input int Take_Profit = 67;                   // Take Profit (pips)

static input string ___0______ = "------";  // --- Bollinger Bands ---
input int Ind0Param0 = 5;                   // Period
input double Ind0Param1 = 2.14;             // Deviation

static input string ___1______ = "------";  // --- Bollinger Bands ---
input int Ind1Param0 = 23;                  // Period
input double Ind1Param1 = 1.73;             // Deviation
...
static input int Max_Spread = 25;             // Max spread (points)
...
static input int Min_Equity = 9100;           // Minimum equity (currency)
...
static input NewsFilterPriority News_Priority =
    NewsFilter_HighOnly;                              // News priority
static input string News_Currencies = "EUR,GBP,USD";  // News currencies
...
string robotTagline = "An Expert Advisor from Top 10 Robots App";
...
int OnInit(void) {
    ...
    if (!MQLInfoInteger(MQL_TESTER)) LoadNews();
...
void OnTick(void) {
    if (!MQLInfoInteger(MQL_TESTER)) {
        ...
        if (time > lastStatsUpdate + 3) {
            ...
            if (Max_OpenPos > sigma || Max_OpenLots > sigma) SetPosStats();
...
void UpdatePosition(void) {
    ...
    for (int posIndex = PositionsTotal() - 1; posIndex >= 0; posIndex -= 1) {
        ...
        if (PositionSelectByTicket(ticket) &&
            ...
            posStopLoss =
                NormalizeDouble(PositionGetDouble(POSITION_SL), _Digits);
            posTakeProfit =
                NormalizeDouble(PositionGetDouble(POSITION_TP), _Digits);
            posPriceOpen = NormalizeDouble(
                PositionGetDouble(POSITION_PRICE_OPEN), _Digits);
            posPriceCurr = NormalizeDouble(
                PositionGetDouble(POSITION_PRICE_CURRENT), _Digits);
            break;
        }
    }
}
...
void InitIndicators(void) {
    // Bollinger Bands (Close, 5, 2.14)
    indHandlers[0][0][0] =
        iBands(NULL, 0, Ind0Param0, 0, Ind0Param1, PRICE_CLOSE);
    if (Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][0][0]);
    // Bollinger Bands (Close, 23, 1.73)
    indHandlers[0][1][0] =
        iBands(NULL, 0, Ind1Param0, 0, Ind1Param1, PRICE_CLOSE);
    if (Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][1][0]);
    // Accelerator Oscillator, Level: 0.0000
    indHandlers[0][2][0] = iAC(NULL, 0);
...
int GetEntrySignal(void) {
    ...
    bool ind0long =
        Open(0) < ind0dnBand1 - sigma && Open(1) > ind0dnBand2 + sigma;
    bool ind0short =
        Open(0) > ind0upBand1 + sigma && Open(1) < ind0upBand2 - sigma;

    // Bollinger Bands (Close, 23, 1.73)
    double ind1buffer0[];
    CopyBuffer(indHandlers[0][1][0], 1, 1, 2, ind1buffer0);
    double ind1buffer1[];
    CopyBuffer(indHandlers[0][1][0], 2, 1, 2, ind1buffer1);
...
void ManageClose(void) {
    ...
    bool ind2long =
        ind2val1 > Ind2Param0 + sigma && ind2val2 < Ind2Param0 - sigma;
    bool ind2short =
        ind2val1 < -Ind2Param0 - sigma && ind2val2 > -Ind2Param0 + sigma;

    if ((posType == OP_BUY && ind2long) || (posType == OP_SELL && ind2short))
        ClosePosition();
}
//+------------------------------------------------------------------+
//|                                                                  |
...
void OpenPosition(const int command) {
    ...
    if (Max_OpenPos > sigma && posStatCount >= Max_OpenPos)
        entryProtectionMessage +=
            StringFormat("Protection: Max open positions: %d, current: %d\n",
                         Max_OpenPos, posStatCount);
    if (Max_OpenLots > sigma && posStatLots > Max_OpenLots - sigma)
        entryProtectionMessage +=
            StringFormat("Protection: Max open lots: %.2f, current: %.2f\n",
                         Max_OpenLots, posStatLots);
    if (Max_Spread > sigma && spread > Max_Spread)
        entryProtectionMessage += StringFormat(
            "Protection: Max spread: %d, current: %d\n", Max_Spread, spread);
    if (MaxDailyLoss > sigma && dailyLoss >= MaxDailyLoss)
        entryProtectionMessage +=
            StringFormat("Protection: Max daily loss: %d, current: %.2f\n",
                         MaxDailyLoss, dailyLoss);
    if (Max_Daily_DD > sigma && dailyDrawdown >= Max_Daily_DD)
        entryProtectionMessage += StringFormat(
            "Protection: Max daily drawdown: %.2f%%, current: %.2f%%\n",
            Max_Daily_DD, dailyDrawdown);
    if (GlobalVariableGet(accEntrySuspendGlobalVarName) > sigma)
        entryProtectionMessage += StringFormat(
            "New entries are suspended until the Daily reset hour: %d",
            Daily_Reset);

    const int newsIndex = NewsFilterActive();
    if (newsIndex > -1) {
        const NewsRecord newsRecord = newsRecords[newsIndex];
        const datetime timeShift =
            (datetime)MathRound((TimeLocal() - TimeGMT()) / 3600.0) * 3600;
        const string priority =
            newsRecord.priority == "high" ? "[high]" : "[med]";
        entryProtectionMessage += StringFormat(
            "News filter: %s %s %s %s\n", priority,
            TimeToString(newsRecord.time + timeShift, TIME_DATE | TIME_MINUTES),
...
//|                                                                  |
//+------------------------------------------------------------------+
void ManageOrderSend(int command, double lots, double stopLoss,
                     double takeProfit, ulong ticket) {
...
double GetTrailingStopPrice(void) {
    ...
    if (posType == OP_BUY) {
        ...
        if (posStopLoss <= newStopLoss - pip)
            return newStopLoss < bid ? newStopLoss >= bid - stopLevelPoints
                                           ? bid - stopLevelPoints
                                           : newStopLoss
                                     : bid;
    }
    ...
    if (posType == OP_SELL) {
        ...
        if (posStopLoss >= newStopLoss + pip)
            return newStopLoss > ask ? newStopLoss <= ask + stopLevelPoints
                                           ? ask + stopLevelPoints
                                           : newStopLoss
                                     : ask;
    }
...
//|                                                                  |
//+------------------------------------------------------------------+
double Bid(void) { return SymbolInfoDouble(_Symbol, SYMBOL_BID); }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double Ask(void) { return SymbolInfoDouble(_Symbol, SYMBOL_ASK); }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
//|                                                                  |
//+------------------------------------------------------------------+
bool IsTradeAllowed(void) { return (bool)MQLInfoInteger(MQL_TRADE_ALLOWED); }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool IsOutOfSession(void) {
    ...
    const int periodFix =
        periodStart + (sessionCloseAtSessionClose ? periodLength : 0);
    const int friBarFix =
...
bool IsForceSessionClose(void) {
    ...
    return dayOfWeek == 0 && sessionCloseAtSessionClose
               ? periodEnd > sessionSundayClose
           : dayOfWeek < 5 && sessionCloseAtSessionClose
               ? periodEnd > sessionMondayThursdayClose
           : dayOfWeek == 5 ? periodEnd > sessionFridayClose
                            : false;
}
//+------------------------------------------------------------------+
//|                                                                  |
...
void UpdateAccountProtection(void) {
    ...
    if (dateTime.hour == Daily_Reset &&
        currentTime - dailyDrawdownLastReset > 60 * 60) {
...
void CheckAccountProtection(void) {
    ...
    if (Min_Equity > sigma && accountEquity <= Min_Equity) {
        ActivateProtection(
            StringFormat("Minimum equity protection activated. Equity: %.2f",
                         accountEquity));
        return;
    }
    ...
    if (Max_Equity > sigma && accountEquity >= Max_Equity) {
        ActivateProtection(
            StringFormat("Maximum equity protection activated. Equity: %.2f",
                         accountEquity));
        return;
    }

    if (MaxEquity_DD > sigma && equityDrawdownPercent >= MaxEquity_DD) {
        ActivateProtection(StringFormat(
            "Max Equity DD protection activated! Equity DD: %.2f%%",
    ...
    if (MaxDailyLoss > sigma && dailyLoss >= MaxDailyLoss) {
        entryProtectionMessage = StringFormat(
            "Max daily loss protection activated! Daily loss: %.2f\n",
            dailyLoss);
        GlobalVariableSet(accEntrySuspendGlobalVarName, 1);
        if (posType == OP_BUY || posType == OP_SELL) ClosePosition();
        return;
    }

    if (Max_Daily_DD > sigma && dailyDrawdown >= Max_Daily_DD) {
        entryProtectionMessage = StringFormat(
            "Max daily drawdown protection activated! Daily DD: %.2f%%\n",
            dailyDrawdown);
        GlobalVariableSet(accEntrySuspendGlobalVarName, 1);
        if (posType == OP_BUY || posType == OP_SELL) ClosePosition();
        return;
...
void UpdateStats(void) {
    ...
    if (entryProtectionMessage != "")
        statsInfo += "\n" + entryProtectionMessage;
    if (Pos_Stat) statsInfo += GetPositionStats() + "\n";
    if (Robot_Stats) statsInfo += GetRobotStats() + "\n";
    if (Max_Spread > sigma || Max_OpenPos > sigma || Max_OpenLots > sigma ||
        MaxDailyLoss > sigma || Max_Daily_DD > sigma || Min_Equity > sigma ||
        Max_Equity > sigma || MaxEquity_DD > sigma)
        statsInfo += GetProtectionInfo();
    if (News_Priority != NewsFilter_Disabled) statsInfo += GetNewsText() + "\n";
...
string GetProtectionInfo(void) {
    ...
    if (Max_Spread > sigma)
        protectionInfo +=
            StringFormat("Max spread: %d, current: %d\n", Max_Spread,
                         (int)MathRound((Ask() - Bid()) / _Point));
    ...
    if (Max_Daily_DD > sigma)
        protectionInfo +=
            StringFormat("Max daily drawdown: %.2f%%, current: %.2f%%\n",
                         Max_Daily_DD, dailyDrawdown);
    if (Min_Equity > sigma)
        protectionInfo +=
            StringFormat("Min equity: %d, current: %.2f\n", Min_Equity,
                         AccountInfoDouble(ACCOUNT_EQUITY));
    if (MaxEquity_DD > sigma)
        protectionInfo +=
            StringFormat("Max equity drawdown: %.2f%%, current: %.2f%%\n",
                         MaxEquity_DD, equityDrawdownPercent);
    if (Max_Equity > sigma)
        protectionInfo +=
            StringFormat("Max equity: %d, current: %.2f\n", Max_Equity,
                         AccountInfoDouble(ACCOUNT_EQUITY));
...
string GetPositionStats(void) {
    ...
    if (posType == OP_FLAT) return positionStats + "Position: no open position";

    return positionStats +
           StringFormat("Position: %s, Lots: %.2f, Profit %.2f\n",
                        (posType == OP_BUY) ? "Long" : "Short", posLots,
                        posProfit) +
...
string GetRobotStatsDays(const int days) {
    ...
    for (int i = 0; i < deals; i += 1) {
        ...
        if (dealMagic > 0 && dealMagic != Magic_Number) continue;
    ...
    const double profitFactor =
        grossLoss > sigma ? grossProfit / grossLoss : grossProfit;
...
//|                                                                  |
//+------------------------------------------------------------------+
string GetNewsInfo(void) { return ""; }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void RenderStats(const string text) {
    ...
    for (int i = 0; i < linesCount; i += 1) {
        ...
        if (lnWidth > lineWidth) lineWidth = lnWidth;
    }
...
{
    if (!ObjectCreate(chartId, name, OBJ_RECTANGLE_LABEL, sub_window, 0, 0))
        return;
    maxRectangles += 1;
...
void InitGlobalVariables(void) {
    ...
    if (!GlobalVariableCheck(accMaxDailyEquityGlobalVarName))
        GlobalVariableSet(accMaxDailyEquityGlobalVarName,
                          AccountInfoDouble(ACCOUNT_EQUITY));
...
string GetNewsContent(string& error) {
    ...
    const int resCode =
        WebRequest("GET", url, "", 10000, reqBody, resData, headers);
...
string GetNewsText(void) {
    ...
    const datetime timeShift =
        (datetime)MathRound((TimeLocal() - timeNow) / 3600.0) * 3600;
    const int newsCount = ArraySize(newsRecords);

    for (int i = 0, count = 0; i < newsCount && count < News_ViewCount;
         i += 1) {
        ...
        const string newsTime =
            TimeToString(newsRecord.time + timeShift, TIME_DATE | TIME_MINUTES);
        const string priority =
            newsRecord.priority == "high" ? "[high]" : "[med]";
        const string text =
            StringFormat("%s%s %s %s %s", newLine, priority, newsTime,
                         newsRecord.currency, newsRecord.title);
...
bool NewsIsAcceptedCurrency(const NewsRecord& newsRecord) {
    for (int i = 0; i < ArraySize(newsCurrencies); i += 1)
        if (newsCurrencies[i] == newsRecord.currency) return true;
...
bool NewsIsAcceptedPriority(const NewsRecord& newsRecord) {
    return (News_Priority == NewsFilter_HighAndMedium) ||
           (News_Priority == NewsFilter_HighOnly &&
            newsRecord.priority == "high");
}
//+------------------------------------------------------------------+
//| Gets the index of an active news or -1                           |
//+------------------------------------------------------------------+
int NewsFilterActive(void) {
    if (News_Priority == NewsFilter_Disabled) return -1;
...
void ParseNewsCurrenciesText(void) {
    ...
    for (int i = 0; i < len; i++) newsCurrencies[i] = parsed[i];
}
//+------------------------------------------------------------------+
/*STRATEGY MARKET BlackBull; EURGBP; M30 */
/*STRATEGY CODE
 * {"properties":{"entryLots":0.01,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":59,"takeProfit":67,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":true},"openFilters":[{"name":"Bollinger
 * Bands","listIndexes":[4,3,0,0,0],"numValues":[5,2.14,0,0,0,0]},{"name":"Bollinger
 * Bands","listIndexes":[1,3,0,0,0],"numValues":[23,1.73,0,0,0,0]}],"closeFilters":[{"name":"Accelerator
